...
def fetch_complaints(make: str, model: str, year: int) -> list:
    """
    Fetches complaints for a single make/model/year combination
    from the NHTSA API.
    """
...
def run_fetch():
    """
    Main entry point.
    Fetches all complaints for all vehicle combinations
    ...
    for i, (make, model, year) in enumerate(combinations, 1):
        logger.info(f"[{i}/{total}] Fetching complaints: {make} {model} {year}")
        rows = fetch_complaints(make, model, year)
        all_rows.extend(rows)

    logger.info(f"Total complaints fetched: {len(all_rows)}")
    ...
    logger.info("Complaints ingestion complete!")
