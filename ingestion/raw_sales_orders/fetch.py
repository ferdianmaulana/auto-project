...
from raw_sales_orders.config import (
    GCP_PROJECT, BQ_DATASET, BQ_TABLE,
    API_ENDPOINT, WRITE_MODE,
)
from raw_sales_orders.schema import SCHEMA

logger = get_logger(__name__)


def fetch_sales_orders(target_date: date = None) -> list:
...
def run_fetch(logical_date: str = None):
    ...
    rows        = fetch_sales_orders(target_date)

    logger.info(f"Total sales orders fetched for {target_date}: {len(rows)}")
    ...
    logger.info("Sales orders ingestion complete!")
